...
// ========== NANO-POW CONFIG ==========
const NANO_POW_ENABLED = process.env.NANO_POW_ENABLED !== '0'; // mặc định bật
const NANO_POW_EFFORT = parseInt(process.env.NANO_POW_EFFORT) || 4;
const NANO_POW_HYBRID = process.env.NANO_POW_HYBRID !== '0'; // mặc định bật hybrid
let nanoPowModule = null; // cache module
...
// ========== LOAD NANO-POW MODULE (LAZY) ==========
async function loadNanoPow() {
    if (!nanoPowModule) {
        try {
            nanoPowModule = await import('nano-pow');
            console.log('✅ nano-pow module loaded');
        } catch (err) {
            console.error('❌ Failed to load nano-pow:', err.message);
            throw new Error('nano-pow is not installed or cannot be imported');
        }
    }
    return nanoPowModule;
}

// ========== GENERATE WORK VIA NANO-POW (HYBRID CPU+GPU) ==========
async function generateWorkWithNanoPow(hash) {
    const { NanoPow } = await loadNanoPow();
    const effort = NANO_POW_EFFORT;
    const hybrid = NANO_POW_HYBRID;

    if (hybrid) {
        // Chạy song song WebGPU và WASM, lấy kết quả nhanh nhất
        const apis = ['webgpu', 'wasm'];
        const tasks = apis.map(api =>
            NanoPow.work_generate(hash, { effort, api })
                .then(r => r.work)
                .catch(() => null) // nếu lỗi, trả về null
        );
        // Dùng Promise.any để lấy kết quả thành công đầu tiên
        const work = await Promise.any(tasks.map(p => p.then(val => {
            if (val === null) throw new Error('null result');
            return val;
        })));
        return work;
    } else {
        // Chế độ đơn (auto)
        const result = await NanoPow.work_generate(hash, { effort });
        if (result && result.work) {
            return result.work;
        }
        throw new Error('No work generated by nano-pow');
    }
}

// ========== GENERATE WORK - FALLBACK TỰ ĐỘNG ==========
async function generateWork(hash) {
    try {
        // Thử Nanswap trước (với retry)
        const result = await nanoRpcCallForTx('work_generate', { hash }, 2);
        if (result && result.work) {
            return result.work;
        }
        // Nếu không có work, ném lỗi để fallback
        throw new Error('No work returned from Nanswap');
    } catch (err) {
        // Nếu nano-pow bị vô hiệu hóa, ném lỗi gốc
        if (!NANO_POW_ENABLED) {
            throw err;
        }
        // Fallback sang nano-pow
        console.warn('⚠️ Nanswap work generation failed, falling back to nano-pow:', err.message);
        try {
            return await generateWorkWithNanoPow(hash);
        } catch (powErr) {
            console.error('❌ Nano-pow fallback also failed:', powErr.message);
            throw powErr;
        }
    }
}
...
// ==================== SET REPRESENTATIVE (SỬ DỤNG generateWork FALLBACK) ====================
let isSettingRepresentative = false;
...
async function setRepresentative(walletData, representativeAddress) {
    ...
    try {
        ...
        const work = await generateWork(blockHash);  // <-- Dùng hàm có fallback
...
async function startApp() {
    ...
    // Thông báo trạng thái nano-pow
    if (NANO_POW_ENABLED) {
        console.log(`💡 nano-pow enabled (hybrid=${NANO_POW_HYBRID}, effort=${NANO_POW_EFFORT})`);
    } else {
        console.log('💡 nano-pow disabled (NANO_POW_ENABLED=0)');
    }
    ...
    app.listen(PORT, () => {
        ...
        console.log(`║  PoW: ${NANO_POW_ENABLED ? 'nano-pow (hybrid)' : 'Nanswap only'}   ║`);
        console.log('╚══════════════════════════════════════╝');
        console.log('');
