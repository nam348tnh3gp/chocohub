...
void sha256_hash(const uchar *msg, uint len, uchar *hash) {
    ...
    // Append length as 64-bit big-endian. Messages here are always far under
    // 2^29 bytes, so total_bits (a 32-bit uint) fits in the low 4 bytes.
    // NOTE: the original code shifted a 32-bit value by 56/48/40/32, which is
    // C undefined behavior and produced garbage length bytes on the Intel NEO
    // compiler, corrupting the hash. Write the 32-bit length big-endian into
    // bytes 60..63 and leave 56..59 as the (zero) high 32 bits.
    for (i = 0; i < 4; i++) {
        block[56 + i] = 0;
    }
    for (i = 0; i < 4; i++) {
        block[60 + i] = (uchar)((total_bits >> (24 - i*8)) & 0xFF);
    }
    sha256_transform(state, block);
...
__kernel void sha256_gpu_miner(
    ...
    // Compare with target (binary): match if hash < target (big-endian, same
    // semantics as Python's hash_hex < target_hex string comparison)
    bool match = false;
    for (int i = 0; i < 32; i++) {
        if (hash[i] < target_hex[i]) {
            match = true;
            break;
        } else if (hash[i] > target_hex[i]) {
            match = false;
            break;
        }
...
class GPUMiner:
    ...
    def init_gpu(self, platform_idx: Optional[int] = None, device_idx: Optional[int] = None) -> bool:
        try:
            ...
            if platform_idx is not None and platform_idx < len(platforms):
                ...
                # Prefer the official Intel(R) OpenCL Graphics platform when
                # present (more reliable than the Mesa rusticl fallback).
                intel_platform = None
                for p in platforms:
                    if "Intel(R) OpenCL Graphics" in p.name:
                        intel_platform = p
                        break
                if intel_platform:
                    self.platform = intel_platform
                else:
                    for p in platforms:
    ...
    def solve_job(self, last_hash_hex: str, target_hex: str, worker_name: str,
                  gpu_load_percent: int = 25, progress_cb=None) -> Tuple[Optional[int], float, float]:
        ...
        target_hex_padded = target_hex.zfill(64)
        target_bytes = bytes.fromhex(target_hex_padded)
        target_np = np.array(list(target_bytes), dtype=np.uint8)
        chunk_items = max(64, int(self.max_work * gpu_load_percent / 100))
        max_nonce = 10000000
        buf_hex = self.cl.Buffer(self.ctx, self.cl.mem_flags.READ_ONLY, size=hex_bytes.nbytes)
        buf_worker = self.cl.Buffer(self.ctx, self.cl.mem_flags.READ_ONLY, size=max(1, worker_bytes.nbytes))
        buf_target = self.cl.Buffer(self.ctx, self.cl.mem_flags.READ_ONLY, size=target_np.nbytes)
        ...
        self.cl.enqueue_copy(self.queue, buf_hex, hex_bytes).wait()
        self.cl.enqueue_copy(self.queue, buf_worker, worker_bytes).wait()
        self.cl.enqueue_copy(self.queue, buf_target, target_np).wait()

        init_val = np.full(1, 0xFFFFFFFF, dtype=np.uint32)
        self.cl.enqueue_copy(self.queue, buf_result, init_val).wait()
...
def detect_gpu_vendor():
    ...
    try:
        ...
        if not gpus:
            return "unknown"
        # Check first GPU's vendor
        vendor = gpus[0]["vendor"].lower()
        name = gpus[0]["name"].lower()
        platform = gpus[0]["platform_name"].lower()
        
        # Check for Intel
        if "intel" in vendor or "intel" in platform or "intel" in name:
            return "intel"
        # Check for NVIDIA
        if "nvidia" in vendor or "nvidia" in platform or "nvidia" in name:
            return "nvidia"
        # Check for AMD/ATI
        if "amd" in vendor or "ati" in vendor or "amd" in platform or "ati" in platform or "amd" in name or "radeon" in name:
            return "amd"
        # Check for Apple (Metal)
        if "apple" in vendor or "apple" in platform:
            return "apple"
        return "unknown"
    except Exception:
        return "unknown"
...
def get_gpu_tier():
    """Map GPU vendor to appropriate tier."""
    vendor = detect_gpu_vendor()
    if vendor == "intel":
        return "gpu"  # Intel integrated/GPU
    elif vendor == "nvidia":
        return "gpu"
    elif vendor == "amd":
        return "gpu"
...
class ChocoMiner:
    def __init__(self, args):
        ...
        # Actual platform/device type reported to the server so its
        # DEVICE_REWARD_MULTIPLIERS can match the vendor (e.g. "intel" for an
        # Intel Arc). Falls back to the tier for CPU/embedded modes.
        self.device_type = getattr(self.args, "tier", "gpu")
        if getattr(self.args, 'gpu', False):
            vendor = detect_gpu_vendor()
            if vendor in ("intel", "nvidia", "amd"):
    ...
    def banner(self):
        ...
        if self.args.gpu:
            ...
            if gpu_vendor != "unknown":
                parts.append(f"{ANSI.GRN}GPU ({gpu_vendor.capitalize()}){ANSI.RST}")
            else:
        ...
        mode_str = " + ".join(parts)
        tier_str = f"  Tier: {ANSI.YEL}{self.args.tier}{ANSI.RST}" if hasattr(self.args, 'tier') else ""
 ...
 ╚██████╗██║  ██║╚██████╔╝╚██████╗╚██████╔╝
  ╚═════╝╚═╝  ╚═════╝  ╚═════╝ ╚═════╝{ANSI.RST}
{ANSI.YEL}        ⚡ Python miner v3 (but with gpu kernel fix) ⚡{ANSI.RST}
{ANSI.GRY}    SHA256(hex_last_hash + 20-digit-nonce + worker){ANSI.RST}
{ANSI.CYN}    Mode: {mode_str}{tier_str}{ANSI.RST}
""")
    ...
    def fetch_job(self):
        try:
            resp = self.session.post(
                ...
                json={"worker_name": self.args.worker, "instance_id": self.instance_id, "device_type": self.device_type},
                timeout=5
            )
            ...
            if not job_id or not last_hash or not target_hex:
                return None
            last_hash = last_hash.zfill(64)
            target_hex = target_hex.zfill(64)
            return {
                "id": job_id,
                "last_hash": last_hash,
                "target_hex": target_hex,
    ...
    def mine_gpu(self):
        ...
        while self.running:
            ...
            def _progress(n_checked):
                with self.stats_lock:
                    self.stats["hashes"] += n_checked

            nonce, hashrate, elapsed = self.gpu_miner.solve_job(
                last_hash, target_hex, self.effective_worker,
                gpu_load_percent=90, progress_cb=_progress
            )
            ...
            if nonce is not None and not self.found_event.is_set():
                if self.stats["current_job"] and self.stats["current_job"]["id"] == jid:
                    ...
                    if hash_hex < target_hex:
                        ...
                        self.found_event.set()
                        with self.stats_lock:
                            self.stats["last_gpu_hashrate"] = int(hashrate)
                        self.log("GPU", f"GPU found valid nonce! Nonce: {nonce} (HR: {hashrate/1e3:.2f} KH/s)", direct=True)
                        last_invalid = None
                    else:
                        if last_invalid == (jid, nonce):
                            self.log("WARN", "GPU stuck on repeated invalid nonce, skipping job", direct=True)
                            time.sleep(0.5)
                        last_invalid = (jid, nonce)
    ...
    def fetch_tier_info(self):
        """Fetch current tier info from server (read-only, replaces deprecated register-tier)."""
        if not self.token:
            return
        tier = getattr(self.args, "tier", None) or "gpu"
        try:
            resp = self.session.get(
                f"{self.args.server}/mining/tier",
            ...
            if resp.status_code == 200 and data.get("status") == "success":
                self.log("OK", (
                    f"Tier: {ANSI.YEL}{data.get('tier')}{ANSI.RST}  "
            ...
            self.log("WARN", f"Tier info unavailable: {e}", direct=True)
    ...
    if is_avr:
        ...
        if ports:
            ...
            try:
                ...
                DEFAULT_ARDUINO_PORT = input(f"  {ANSI.YEL}➤ Enter port manually (e.g. /dev/ttyACM0, COM3){ANSI.RST}: ").strip()
        else:
            DEFAULT_ARDUINO_PORT = input(f"  {ANSI.YEL}➤ Enter port (e.g. /dev/ttyACM0, COM3){ANSI.RST}: ").strip()
        ...
        if not is_mobile:
            ...
            print(f"     3) {ANSI.MAG}GPU only{ANSI.RST} (no CPU mining)")
            if DEFAULT_GPU_ONLY:
                gpu_hint = " [3]"
            elif DEFAULT_GPU:
                gpu_hint = " [2]"
                ...
                gpu_hint = " [1]"
            gpu_choice = input(f"  {ANSI.YEL}➤ Choice (1/2/3){ANSI.RST}{gpu_hint}: ").strip()
            if not gpu_choice:
                use_gpu = DEFAULT_GPU
                gpu_only = DEFAULT_GPU_ONLY
            else:
                use_gpu = (gpu_choice in ("2", "3"))
                gpu_only = (gpu_choice == "3")
            DEFAULT_GPU = use_gpu
            if gpu_only:
                DEFAULT_THREADS = 0
            # Auto-detect GPU vendor and set tier
            if use_gpu and '--tier' not in sys.argv:
                detected_vendor = detect_gpu_vendor()
                if detected_vendor != "unknown":
                    print(f"  {ANSI.GRY}Detected GPU: {detected_vendor.capitalize()}{ANSI.RST}")
                    # All GPU vendors use 'gpu' tier on server
...
def parse_arguments():
    ...
    parser.add_argument("--gpu-only", action="store_true", default=False, help="GPU only mode (disables CPU mining)")
    parser.add_argument("--arduino-port", default=DEFAULT_ARDUINO_PORT, help="Serial port for Arduino (e.g. /dev/ttyACM0, /dev/ttyUSB0 on Linux, COM3 on Windows)")
...
def main():
    ...
    DEFAULT_GPU_ONLY      = config.get("gpu_only", False)
    ...
    has_essential = any(x in args_passed for x in [
        '--worker', '--threads', '--gpu', '--no-gpu', '--gpu-only', '--poll',
    ...
    if '--tier' not in args_passed:
        if args.arduino_port and not args.gpu:
            args.tier = "embedded_avr"
        elif args.gpu:
            args.tier = get_gpu_tier()
        else:
    ...
    # --gpu-only implies --gpu and disables CPU threads
    if args.gpu_only:
        args.gpu = True
        args.threads = 0
